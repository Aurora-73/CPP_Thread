...
// 线程 与 mutex
/*
std::thread 启动函数时忽略返回值，线程结束后返回值直接销毁；如需获取结果，应使用 std::future / std::async 或 promise。
...
this_thread::yield： 向实现提供一个提示，以重新调度线程的执行，允许其他线程运行。例如先进先出实时调度器会暂停当前线程并将其置于相同优先级就绪运行线程队列的末尾，如果没有其他相同优先级的线程，则 yield 没有效果。
*/
...
// +1操作过于简单，如果LoopCount不足够大，无法暴露错误
...
int main() {
...

	{
		int a = 0;
		for(int i = 0; i < 5; ++i) {
			std::thread(foo1, std::ref(a), std::ref(mtx)).join();
		}
		cout << "Serial final value of a: " << a << endl;
	} // 这里本质上是串行运行的，因为join是阻塞到子进程运行结束，因此声明后直接join实际上是串行
