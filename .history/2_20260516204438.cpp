...
unique_lock  实现可移动的互斥体所有权包装器 (类模板)
    ...
    构造函数：
        ...
        unique_lock( mutex_type& m, std::try_to_lock_t t ); 用pmutex引用m，owns = mtx.try_lock()，尝试加锁
        unique_lock( mutex_type& m, std::adopt_lock_t t ); 接管已经加好的锁，用pmutex引用m，owns设置true，默认已经加锁，不再次加锁
    析构函数：
        如果拥有，则解锁（即释放）关联的互斥量
    operator=
...
int main() {
	mutex mtx;
	unique_lock<mutex> ul(mtx, defer_lock_t t);
}