...
辅助函数：
    ...
    void call_once( std::once_flag& flag, Callable&& f, Args&&... args );
        ...
        如果，当调用std::call_once时，flag指示f已被调用过，std::call_once会立即返回。
        否则，std::call_once会调用f(args)。与std::thread构造函数或std::async不同，参数不会被移动或复制，因为它们不需要转移到另一个执行线程。
        如果该调用抛出异常，则该异常会传播到std::call_once的调用者，并且flag不会被翻转，以便再次尝试调用。
        如果该调用正常返回，则flag会被翻转。
