...
void foo_bad(int &a, mutex &mtx) {
	lock_guard<mutex> {
	    mtx}; // ignoring return value of 'std::lock_guard<_Mutex>::lock_guard(mutex_type&) [with _Mutex = std::mutex; mutex_type = std::mutex]', declared with attribute 'nodiscard' [-Wunused-result]gcc
	// Wring的意思是 这个对象不该被忽略。因为：忽略几乎一定写错了。
