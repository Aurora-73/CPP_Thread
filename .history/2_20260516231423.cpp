...
void foo_two1(mutex &mtx1, mutex &mtx2) {
	mtx1.lock();
	this_thread::yield();
	mtx2.lock();
	this_thread::yield();
...
void foo_two2(mutex &mtx1, mutex &mtx2) {
	mtx2.lock();
	this_thread::yield();
	mtx1.lock();
	this_thread::yield();
...
int main() {
	...
	{
		mutex mtx1, mtx2;
		std::thread t1(foo_two1, std::ref(mtx1), std::ref(mtx2));
		std::thread t2(foo_two2, std::ref(mtx2), std::ref(mtx1));
		t1.join();
		t2.join();
		std::thread t3(foo_two1, std::ref(mtx1), std::ref(mtx2));
		std::thread t4(foo_two2, std::ref(mtx2), std::ref(mtx1));
		t3.join();
		t4.join();
		std::thread t5(foo_two1, std::ref(mtx1), std::ref(mtx2));
		std::thread t6(foo_two2, std::ref(mtx2), std::ref(mtx1));
		t5.join();
		t6.join();
		std::thread t7(foo_two1, std::ref(mtx1), std::ref(mtx2));
		std::thread t8(foo_two2, std::ref(mtx2), std::ref(mtx1));
		t7.join();
		t8.join();
		{
			std::thread t1(foo_two1, std::ref(mtx1), std::ref(mtx2));
		...
		cout << "woring using lock_guard" << endl;
	}
}