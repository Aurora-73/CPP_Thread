...
bool deadlock = false;
void foo_two_bad(int i, mutex &mtx1, mutex &mtx2) {
	if(deadlock) return;
	mtx1.lock();
	mtx2.lock();
	if(!deadlock) cout << i << " ";
...
int main() {
...

	{
		...
		while(!mtx1.try_lock() && !mtx2.try_lock()) {
			deadlock = true;

			mtx1.unlock(), mtx2.unlock();
			this_thread::sleep_for(10ms);
		}
		if(deadlock = true) cout << '\n' << "deadlock happend, release lock" << endl;
		mtx1.unlock(), mtx2.unlock();
		for(auto &t : tv) t.join();
		cout << '\n' << "woring using" << endl;
