...
int main() {
...

	{
...

		{
			...
			try {
				...
				cout << "对于_M_owns为true的ul调用lock会抛出异常，即使实际上mutex未上锁" << endl;
			}
			cout << "adopt_lock: " << (mtx.try_lock() ? "不会给mtx加锁" : "会给mtx加锁") << "; "
			     << (ul ? "ul拥有所有权" : "ul没有所有权") << endl;
...

	{
		...
		cout << "multiple_mutex woring used" << endl;
