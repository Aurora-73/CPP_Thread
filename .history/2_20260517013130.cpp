...
int main() {
...

	{
...

		{
			...
			cout << "defer_lock: " << (mtx.try_lock() ? "mtx未上锁" : "mtx已经上锁") << "; "
			     << (ul.owns_lock() ? "ul拥有所有权" : "ul没有所有权") << endl; // defer_lock构造不加锁，无所有权
			mtx.unlock();

			ul.lock();
			cout << "    加锁之后: " << (mtx.try_lock() ? "mtx未上锁" : "mtx已经上锁") << "; "
			     << (ul.owns_lock() ? "ul拥有所有权" : "ul没有所有权") << endl; // 加锁之后拥有所有权
			cout << "    内部指针：" << &mtx << " " << ul.mutex() << " " << ul.release() << endl;
			cout << "    释放之后: " << (mtx.try_lock() ? "mtx未上锁" : "mtx已经上锁") << "; "
			     << (ul ? "ul拥有所有权" : "ul没有所有权") << "内部指针: " << ul.mutex() << endl;
			// 释放不解锁，但是没有所有权，且指针置0
			try {
				ul.lock();
			} catch(exception &e) {
...

		{
			...
			cout << "try_to_lock: " << (mtx.try_lock() ? "不会给mtx加锁" : "会给mtx加锁") << endl;
			cout << "    尝试加锁成功，" << (ul1 ? "ul1拥有所有权" : "ul1没有所有权") << endl;
			cout << "    尝试加锁失败，" << (ul2 ? "ul2拥有所有权" : "ul2没有所有权") << endl;
			// try_to_lock 构造尝试加锁，但是拥有所有权，是接管已经有的所有权
		}
	} // unique_lock的常见用法
...

	{
		...
		vector<thread> tv;
		for(int i = 0; i < 100; ++i) {
			tv.emplace_back(foo_two_bad, 2 * i, ref(mtx1), ref(mtx2));
			tv.emplace_back(foo_two_bad, 2 * i + 1, ref(mtx2), ref(mtx1));
		}
		this_thread::sleep_for(2s);
		cout << '\n' << "deadlock, multiple_mutex wrong used" << endl;
		for(auto &t : tv) t.join();
