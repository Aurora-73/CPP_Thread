...
void foo_scoped_lock(int i, mutex &mtx1, mutex &mtx2) {
	...
	// 或者 lock(mtx1, mtx2); 然后 mtx1.unlock(), mtx2.unlock();
	cout << i << " ";
}
...
void foo_two_timeout(int i, timed_mutex &m1, timed_mutex &m2) {
	unique_lock<timed_mutex> left(m1);
	this_thread::sleep_for(std::chrono::milliseconds(1));

	unique_lock<timed_mutex> right(m2, defer_lock);
	if(!right.try_lock_for(std::chrono::milliseconds(50))) {
		cout << "task " << i << " lock timeout" << endl;
		return;
	}
...
void shared_reader(int id, int &value, shared_mutex &smtx) {
	for(int i = 0; i < 3; ++i) {
		shared_lock sl(smtx);
...
void shared_writer(int &value, shared_mutex &smtx) {
	for(int i = 0; i < 3; ++i) {
		unique_lock ul(smtx);
		++value;
...
void foo_once() {
	call_once(onceFlag, [] {
		cout << "call_once execute once" << endl;
		++onceValue;
	});
}
...
int main() {
...

	{
		...
		t1.join();
		t2.join();
		cout << "wrong using temporary lock_guard, a = " << a << endl;
...

	{
...

		{
			...
			cout << "默认构造: " << (ul.owns_lock() ? "ul拥有所有权" : "ul没有所有权") << endl;
			// 默认构造立刻加锁，并且拥有所有权
		}

		{
			unique_lock<mutex> ul(mtx, defer_lock);
			cout << "defer_lock: " << (ul.owns_lock() ? "ul拥有所有权" : "ul没有所有权") << endl;
			// defer_lock构造不加锁，无所有权

			ul.lock();
			cout << "    加锁之后: " << (ul.owns_lock() ? "ul拥有所有权" : "ul没有所有权") << endl;
			cout << "    内部指针：" << &mtx << " " << ul.mutex() << endl;
			mutex *released = ul.release();
			cout << "    release返回：" << released << endl;
			cout << "    释放之后: " << (ul ? "ul拥有所有权" : "ul没有所有权") << "，内部指针: " << ul.mutex() << endl;
			// 释放不解锁，但是没有所有权，且指针置0
			try {
				ul.lock();
			} catch(exception &e) {
		...
		{
			mtx.lock(); // adopt_lock前提：当前线程已经持有锁
			unique_lock<mutex> ul(mtx, adopt_lock);
			try {
				ul.lock();
			} catch(exception &e) {
				cout << "对于_M_owns为true的ul调用lock会抛出异常" << endl;
			}
			cout << "adopt_lock: " << (ul ? "拥有所有权" : "没有所有权") << endl;
			// adopt_lock 构造不加锁，但是拥有所有权，是接管已经有的所有权
		}

		{
			unique_lock<mutex> ul1(mtx, try_to_lock);
			unique_lock<mutex> ul2(mtx, try_to_lock);
			cout << "try_to_lock:" << endl;
			cout << "    第一次尝试，" << (ul1 ? "ul1拥有所有权" : "ul1没有所有权") << endl;
			cout << "    第二次尝试，" << (ul2 ? "ul2拥有所有权" : "ul2没有所有权") << endl;
			// try_to_lock 构造会尝试加锁；只有加锁成功的对象才拥有所有权
		}
	} // unique_lock的常见用法
...

	{
		shared_mutex smtx;
		int value = 0;
		thread writer(shared_writer, ref(value), ref(smtx));
		thread reader1(shared_reader, 1, ref(value), ref(smtx));
		thread reader2(shared_reader, 2, ref(value), ref(smtx));
		writer.join();
		reader1.join();
		reader2.join();
	} // shared_lock，施加读锁后尝试获取写锁会阻塞，施加写锁后尝试获取读写锁都会阻塞
...

	{
		vector<thread> tv;
		for(int i = 0; i < 5; ++i) {
			tv.emplace_back(foo_once);
		}
		for(auto &t : tv) t.join();

		cout << "onceValue = " << onceValue << endl;
	}
