...
public:
	...
	bool should_stop_producing() const {
		return stop_produce_.load();
	}
	...
	bool push_task(std::string task) {
		...
		cv_not_full_.wait(lk, [this]() { return tasks_.size() < max_size || stop_produce_.load(); });

		// 停止生产后，不再接收新任务，避免停机时继续扩张任务量。
		if(stop_produce_.load()) {
			return false;
		}
	...
	bool pop_task(std::string &task) {
		std::unique_lock<std::mutex> lk(mtx_);
		cv_not_empty_.wait(lk, [this]() { return !tasks_.empty() || active_producers_.load() == 0; });

		// 没有生产者且队列已空，说明系统中的任务已经全部处理完成。
		if(tasks_.empty() && active_producers_.load() == 0) {
			return false;
		}
...
private:
	...
	std::atomic<bool> stop_produce_ = false;
...
void producer(int i) {
	while(!task_queue.should_stop_producing()) {
		int tid;
		{
